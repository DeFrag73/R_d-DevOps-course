...
1. [TASK № 1: Робота з віртуальними машинами в VirtualBox](#TASK-1)
2. [TASK № 2: Vagrant Multi-VM Setup для вебсерверів](#TASK-2)
3. [TASK № 3: Встановлення та налаштування Nginx, Systemd сервісу, UFW та Fail2Ban](#TASK-3)
4. [TASK № 4: Автоматична установка пакетів для різних дистрибутивів Linux](#TASK-4)
...
1. **Встановлення віртуальної машини**
   ...
   - ![Встановлення.png](task_1_Virtualization/Встановлення.png)

2. **Початкові налаштування ВМ**
   - Виконано початкові налаштування, такі як конфігурація процесора, оперативної пам'яті, та диску.
   - ![Початкові налаштування](task_1_Virtualization/початкові налаштування ВМ.png)

3. **Збільшення ресурсів ВМ**
   - **Оперативна пам'ять**: Було збільшено кількість оперативної пам'яті.
     - ![Збільшуємо оперативну пам'ять](task_1_Virtualization/збільшуємо оперативну пам'ять.png)
   - **Кількість ядер процесора**: Було збільшено кількість ядер процесора для ВМ.
     - ![Збільшуємо кількість ядер процесора](task_1_Virtualization/збільшуємо ядер процесора.png)

4. **Збільшення розміру диска**
   - Диск віртуальної машини було збільшено, щоб відповідати вимогам для зберігання даних.
   - ![Змінюємо розмір диску](task_1_Virtualization/змінюємо розмір диску.png)
   - Спочатку диск виглядав так:
     - ![Диск до оновлення](task_1_Virtualization/диск в системі до оновлення.png)
   - Після розмітки за допомогою GParted диск було розширено:
     - ![Розмітка диска за допомогою GParted](task_1_Virtualization/за допомоги gparted розмічуємо диск.png)
     - ![Диск після оновлення](task_1_Virtualization/діск розширено.png)

5. **Збереження стану ВМ**
   - Виконано збереження поточного стану віртуальної машини за допомогою створення снапшоту.
   - ![Робимо снапшот](task_1_Virtualization/робимо снапшот.png)

6. **Видалення віртуальної машини**
   - Система після видалення виглядала так:
     - ![Після видалення](task_1_Virtualization/нічого не працює після видалення.png)
   - Після видалення машина була успішно відновлена до збереженого стану.
     - ![Машина відновлена](task_1_Virtualization/машина повністю відновлена.png)
7. **Відновлення ВМ**
   
   - Було видалено віртуальну машину для перевірки процесу відновлення.
     - ![Видалення машини](task_1_Virtualization/видаляємо машину.png)
## Висновок

У цій роботі ми пройшли через процес створення, налаштування, збільшення ресурсів, збереження та відновлення віртуальної машини, а також зміни розміру диску за допомогою GParted.
У розділенні диску можна було розшири наявний розділ, а не розмічати новий, але завтикав, а переробляти не хотів
